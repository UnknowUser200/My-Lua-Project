...
function lib:Window(text, preset, closebind)
    ...
    SearchBox.Text = ""
    SearchBox.TextColor3 = Color3.fromRGB(178, 178, 178)
    ...
    function SearchConstructor()
        function StitchElements()
            local elms = {}
            for i,v in next, lib.CurrentTab:GetDescendants() do
        ...
        SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
            for i,v in pairs(StitchElements()) do
                if string.len(SearchBox.Text) >= 1 then
                    if string.find(string.sub(v.Name:lower(),0,-8), SearchBox.Text:lower()) then
                        v.Visible = true
                    else
                        v.Visible = false
                    end 
