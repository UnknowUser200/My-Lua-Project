...
function UILibrary:Main(PrjName, HideKey)
    ...
    local function update(input)
        ...
        pcall(function()
            Main:TweenPosition(UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X, startPos.Y.Scale,
                startPos.Y.Offset + delta.Y), "Out", "Quad", 0.1, true, nil)
        end)
    end
        ...
        spawn(function()
            for _ = 1, 5 do wait(0.7)
                ...
                for i, v in pairs(obj) do
                    ...
                    if _ == 5 then
                        TweenService:Create(FadeBackgroundFrame, TweenInfo.new(0.3), { BackgroundTransparency = 1 }):
                            Play()
    ...
    function Tabs:Tab(TabName)
        ...
        for index, page in next, Pages:GetChildren() do
            if page.Name ~= "UIPageLayout" and (page.Name == Frame.Name and index == 2) then
                ...
                TweenService:Create(TabButton, TweenInfo.new(0.25),
                    { BackgroundTransparency = 0.3, BackgroundColor3 = Color3.fromRGB(1, 124, 255) }):Play()
        ...
        function ElementsLib:Section(SectionName)
            ...
            function InSection:Slider(SliderText, min, max, start, inc, callback)
                ...
                local function move(Input)
                    local XSize = math.clamp((Input.Position.X - Sliding.AbsolutePosition.X) / Sliding.AbsoluteSize.X, 0
                        , 1)
                    local Increment = inc and (max / ((max - min) / (inc * 4))) or
                        (max >= 50 and max / ((max - min) / 4)) or (max >= 25 and max / ((max - min) / 2)) or
                        (max / (max - min))
                    local SizeRounded = UDim2.new((math.round(XSize * ((max / Increment) * 4)) / ((max / Increment) * 4)
                        ), 0, 1, 0)
                    TweenService:Create(Sliding, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                ...
                function Slider:Set(val)
                    ...
                    ValueLabel.Text = tostring(a)
                    Progress.Size = UDim2.new((val or 0) / max, 0, 1, 0)
                    Slider.Value = val
            ...
            function InSection:Dropdown(placeholder, Selectables, func, IsMulti)
                ...
                local function NewSelectable(Text, val)
                        ...
                        if not IsMulti then
                            ...
                            for i, v in next, DropdownList:GetDescendants() do
                                if v.Name == "IsChoosedVal" and v.Parent.Name ~= Text then
                                    v.Value = nil
                                    TweenService:Create(v.Parent.ChoosedCircle.CircleStroke, TweenInfo.new(0.15),
                                        { Thickness = 1.3 }):Play()
                                    TweenService:Create(v.Parent.ChoosedCircle, TweenInfo.new(0.15),
                                        { BackgroundTransparency = 1 }):Play()
                                    v.Parent.ChoosedCircle.InnerCircle:TweenSize(UDim2.new(0, 0, 0, 0), "Out", "Sine",
                                        0.15, true)
                                    TweenService:Create(v.Parent, TweenInfo.new(0.25),
                            ...
                            if IsSelectableChoosed.Value then
                                ...
                                TweenService:Create(ChoosedCircle, TweenInfo.new(0.15), { BackgroundTransparency = 1 }):
                                    Play()
                                TweenService:Create(ChoosedCircleStroke, TweenInfo.new(0.15), { Thickness = 1.3 }):Play()
                            else
                                IsSelectableChoosed.Value = true
                                InnerCircle:TweenSize(UDim2.new(0, 12, 0, 12), "In", "Sine", 0.15, true)
                                TweenService:Create(ChoosedCircle, TweenInfo.new(0.15), { BackgroundTransparency = 0 }):
                                    Play()
                                TweenService:Create(ChoosedCircleStroke, TweenInfo.new(0.15), { Thickness = 0 }):Play()
                            end
                        end
                    ...
                    SelectableButton.MouseEnter:Connect(function()
                        TweenService:Create(SelectableButton, TweenInfo.new(0.25), { BackgroundTransparency = 0.96 }):
                            Play()
                    end)
            ...
            function InSection:Button(btnText, func)
                ...
                Button.MouseButton1Click:Connect(function()
                    ...
                    if not IsMouseOn then
                        ...
                        TweenService:Create(Button, TweenInfo.new(0.25),
                            { BackgroundTransparency = 0.98, BackgroundColor3 = Color3.fromRGB(255, 255, 255) }):Play()
                    end
                end)
